...
#构建句子的特征矩阵，不包括距离特征。将特征矩阵提取到source_wzp_lda_1.txt,然后PMI筛选特征
...
def buildvector():
    ...
    file = open("data\\vectoritems_lda_3.txt", "r")
...
def main():
    ...
    #将特征矩阵提取到source_wzp_lda_1.txt
    vectorfile = open("data\\source_wzp_lda_1.txt", "w")
    sentencefile = open("data\\sentences.txt".decode('utf-8'), "r")

    word = open("data\\vectoritems_lda_3_new.txt","w")
